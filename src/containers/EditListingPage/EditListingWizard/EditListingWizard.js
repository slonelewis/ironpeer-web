...
const tabsForListingType = (processName, listingTypeConfig) => {
  ...
  const tabs = {
    ['default-booking']: [DETAILS, PROTECTION, ...locationMaybe, PRICING, ...styleOrPhotosTab, AVAILABILITY, DELIVERY],
...
const tabLabelAndSubmit = (intl, tab, isNewListingFlow, isPriceDisabled, processName) => {
  ...
  if (tab === PROTECTION) {
    labelKey = 'EditListingWizard.tabLabelProtection';
    submitButtonKey = `EditListingWizard.${processNameString}${newOrEdit}.saveProtection`;
  } else if (tab === DETAILS) {
